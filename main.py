...
forward_sound = pygame.mixer.Sound("Forward.ogg")
shot_sound = pygame.mixer.Sound("Shot3.ogg")
collision_sound = []
collision_sound.append(pygame.mixer.Sound("Explosion2_1.ogg"))
collision_sound.append(pygame.mixer.Sound("Explosion2_2.ogg"))
collision_sound.append(pygame.mixer.Sound("Explosion2_3.ogg"))
...
def Gameinit():
    ...
    def bullet_collision(bullet: Bullet, comets):
        ...
        if collider:
            collision_sound[collider.lvl-1].play()
            Score += 1 #Increments score for each comet kill
            bullet.kill()
            comets_spawn(collider)
            if collider.lvl==3:
                #keeps track of level2 and level3 comets that spawned from a particular level1 comet by reducing respective entry counter from its current value
                comet_level1_trailer[collider.level1_ID] = comet_level1_trailer[collider.level1_ID] - 1
                #if counter reaches 0 then a new level1 comet is spawned
    ...
    # function to wrap around the screen over the movement/position of the comets and player    
    ...
    #array to keep track of the nr of existing comets spawned from level1 comets - each entry referes to a new level1 comet and has a counter that goes down from 15 to 0.
    #when 0 is reached on any of the counters, another level1 comet is spawned!
...
while True:
    ...
    pygame.mixer.music.load("SynthOrb - Blockade.mp3")
    pygame.mixer.music.play(loops=-1)
